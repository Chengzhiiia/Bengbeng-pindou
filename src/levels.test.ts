...
describe('levels', () => {
  ...
  it('uses the cropped pixel-art bathing dog reference as the second level target pattern', () => {
    ...
    expect(cellAt(secondLevel, 8, 1)?.targetColor).toBe('#ffe0a6')
    expect(cellAt(secondLevel, 0, 14)?.targetColor).toBe('#8fd0ff')
    expect(cellAt(secondLevel, 16, 15)?.targetColor).toBe('#ff2d2d')
    expect(cellAt(secondLevel, 15, 14)?.targetColor).toBe('#ffe06a')
    expect(cellAt(secondLevel, 8, 9)?.targetColor).toBe('#8f4c00')
  ...
  it('uses the cropped pixel-art late-night dog reference as the third level target pattern', () => {
    ...
    expect(cellAt(thirdLevel, 6, 0)?.targetColor).toBe('#f0d83f')
    expect(cellAt(thirdLevel, 13, 6)?.targetColor).toBe('#111111')
    expect(cellAt(thirdLevel, 7, 11)?.targetColor).toBe('#fffdf5')
    expect(cellAt(thirdLevel, 16, 8)?.targetColor).toBe('#8d4f24')
    expect(cellAt(thirdLevel, 12, 6)?.targetColor).toBe('#c47a2b')
